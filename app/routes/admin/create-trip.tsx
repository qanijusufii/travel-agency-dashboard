...
export const loader = async () => {
    ...
    return data.map((country: { flag: any; name: { common: any; }; latlng: any; maps: { openStreetMap: any; }; }) => ({
...
const CreateTrip = ({ loaderData }: Route.ComponentProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        //Checking if the user has filled all the necessary fields!
        if(!formData.country ||
            !formData.travelStyle ||
            !formData.interest ||
            !formData.budget ||
            !formData.groupType) {
            setError('Please provide values from all fields');
            setLoading(false);
            return;
        }

        if(formData.duration < 1 || formData.duration > 10) {
            setError('Duration must be betwwen 1 and 10 days');
            setLoading(false);
            return;
        }
